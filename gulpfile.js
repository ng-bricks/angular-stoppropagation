...
gulp.task('scripts', function() {
  ...
  function buildTemplates() {
    return gulp.src('src/**/*.html')
      ...
      .pipe(templateCache({module: 'ng-tricks.ng-stoppropagation'}));
  };

  function buildDistJS(){
    return gulp.src('src/ng-stoppropagation.js')
      .pipe(plumber({
  ...
  es.merge(buildDistJS(), buildTemplates())
    ...
    .pipe(order([
      'ng-stoppropagation.js',
      'template.js'
    ]))
    .pipe(concat('ng-stoppropagation.js'))
...
gulp.task('styles', function() {
  ...
  return gulp.src('src/ng-stoppropagation.less')
    .pipe(less())
    .pipe(header(config.banner, {
      timestamp: (new Date()).toISOString(), pkg: config.pkg
    }))
